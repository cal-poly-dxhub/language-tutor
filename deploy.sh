...
#
# The stack builds container images (the Nova Sonic bridge, and — unless
# `-c phonemes=false` — the Wav2Vec2 SageMaker model) and bundles the converse Lambda in
# a build container, so a deploy needs a working builder. The CDK CLI defaults to
# whatever `docker` binary is on PATH — and on a Mac with the Docker CLI installed but no
# Docker Desktop daemon running, the build fails with:
#
...
#   2. puts ./bin first on PATH, where a `docker` shim forwards to finch — so even a code
#      path that ignores CDK_DOCKER and shells out to `docker` still works.
#
# Usage:
